...
class ImageLoader(context: Context) {
    ...
    //val stub_id: Int = R.drawable.
    fun DisplayImage(url: String, imageView: ImageView ,defaulturl :Int) {
        ...
        if (bitmap != null) {
            ...
            queuePhoto(url, imageView,defaulturl)

            //Before downloading image show default image
           // imageView.setImageResource(defaulturl)
        }
    }

    private fun queuePhoto(url: String, imageView: ImageView ,defaulturl: Int) {
        ...
        executorService.submit(PhotosLoader(p,defaulturl))
    }
    ...
    internal inner class PhotosLoader(var photoToLoad: PhotoToLoad,var defaulturl: Int) : Runnable {
        override fun run() {
            try {
                ...
                val bd = BitmapDisplayer(bmp, photoToLoad,defaulturl)
    ...
    internal inner class BitmapDisplayer(var bitmap: Bitmap?, var photoToLoad: PhotoToLoad,var defaulturl: Int) :
        Runnable {
        override fun run() {
            if (imageViewReused(photoToLoad)) return
